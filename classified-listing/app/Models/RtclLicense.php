...
class RtclLicense {
	...
	public function init() {
		...
		add_action( 'after_plugin_row_' . $this->name, [ $this, 'show_update_notification_key_check' ], 10, 2 );
	...
	public function show_update_notification( $file, $plugin ) {
		...
		if ( ! empty( $update_cache->response[ $this->name ] ) && version_compare( $this->version, $version_info->new_version, '<' ) ) {
			...
			if ( empty( $version_info->download_link ) ) {
				printf(
				/* translators: Version quality. 1: version number, 2: version number. 3: version number 4: detail link */
					esc_html__( 'There is a new version of %1$s available. %2$sView version %3$s details%4$s.', 'classified-listing' ),
				...
				printf(
				/* translators: Version quality. 1: version number, 2: version number. 3: version number 4: detail link  5: Update link, 6: link */
					esc_html__( 'There is a new version of %1$s available. %2$sView version %3$s details%4$s or %5$supdate now%6$s.', 'classified-listing' ),
					esc_html( $version_info->name ),
					'<a target="_blank" class="thickbox" href="' . esc_url( $changelog_link ) . '">',
					esc_html( $version_info->new_version ),
					'</a>',
