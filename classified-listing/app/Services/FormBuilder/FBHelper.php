...
class FBHelper {
	...
	public static function getFieldAttachmentFiles( $listing_id, $field ) {
				...
				if ( 'title' === $element ) {
					...
					if ( $listing->get_form_id() ) {
						if ( !empty( $rawBsh['active'] ) ) {
							...
							$bhs = [
								'active' => true,
								'type'   => !empty( $rawBsh['type'] ) && $rawBsh['type'] === 'selective' ? 'selective' : 247,
	...
	public static function sanitizeFieldValue( $rawValue, $field ) {
		...
		switch ( $element ) {
			...
			case 'textarea':
				if ( !empty( $field['editor_type'] ) && 'wp_editor' === $field['editor_type'] ) {
					...
					$sanitize_value = esc_textarea( trim( $rawValue ) );
				}
				break;
			case 'html':
				$sanitize_value = wp_kses_post( $rawValue );
			...
			case 'repeater':
				...
				break;
			default:
				$sanitize_value = sanitize_text_field( trim( $rawValue ) );
				break;
		}
	...
	static function getFormFieldByUuid( $uuid, $listing_id = null, $form_id = null ) {
		...
		if ( !empty( $form_id ) ) {
			$form = Form::query()->select( 'fields,id,title,slug' )->find( $form_id )->toArray();
		} else {
			$form = Form::query()->select( 'fields,id,title,slug' )->find( 1, 'default' )->toArray();
		}
	...
	public static function getWeekDays() {
		...
		return array_map( function ( $_item, $_itemIndex ) {
			return [
				'day'      => $_item,
				'dayIndex' => $_itemIndex
			];
		...
		if ( $field->getElement() === 'color_picker' ) {
			...
			if ( !empty( $value ) && is_array( $value ) ) {
				...
				foreach ( $value as $file ) {
					...
					} else if ( str_starts_with( $mime_type, 'audio/' ) ) {
					...
					} else if ( str_starts_with( $mime_type, 'application/' ) ) {
						$ext = explode('/', $mime_type)[1];
						if ( $ext === 'pdf'  ) {
							$iconClass = 'rtcl-icon-file-pdf';
						}else if ( in_array( $ext, [ 'zip', 'gz', 'gzip', 'rar', '7z' ] ) ) {
