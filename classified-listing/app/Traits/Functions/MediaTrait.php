...
trait MediaTrait {
	...
	/**
	 * Resolve usable pixel dimensions for an attachment at a given image size.
	 *
	 * WordPress reports the constrained "1x1" dimensions whenever the requested size can not be
	 * resolved properly, e.g. the sub size was never generated, the attachment metadata is broken
	 * or the size itself is registered with an invalid width/height. Old installs hold plenty of
	 * such attachments, so resolve the best known dimensions rather than trusting the size name.
	 *
	 * @param int          $attachment_id Attachment ID.
	 * @param string|int[] $size          Registered image size name or [ width, height ].
	 *
	 * @return int[] [ width, height ]. Zeros when nothing usable could be resolved.
	 */
	static function get_attachment_image_dimensions( $attachment_id, $size = 'rtcl-thumbnail' ) {
		...
		$full_w = ! empty( $meta['width'] ) ? absint( $meta['width'] ) : 0;
		$full_h = ! empty( $meta['height'] ) ? absint( $meta['height'] ) : 0;

		// 1. The generated sub size, when it is really there.
		if ( is_string( $size ) && ! empty( $meta['sizes'][ $size ]['width'] ) && ! empty( $meta['sizes'][ $size ]['height'] ) ) {
			$sub_w = absint( $meta['sizes'][ $size ]['width'] );
			$sub_h = absint( $meta['sizes'][ $size ]['height'] );
			if ( $sub_w > 1 && $sub_h > 1 ) {
				return [ $sub_w, $sub_h ];
			}
		}

		// 2. The size definition itself, scaled down to the original image.
		$max_w = 0;
		$max_h = 0;

		if ( is_array( $size ) ) {
			$max_w = ! empty( $size[0] ) ? absint( $size[0] ) : 0;
			$max_h = ! empty( $size[1] ) ? absint( $size[1] ) : 0;
		} else {
			$registered = wp_get_registered_image_subsizes();
			if ( ! empty( $registered[ $size ] ) ) {
				$max_w = ! empty( $registered[ $size ]['width'] ) ? absint( $registered[ $size ]['width'] ) : 0;
				$max_h = ! empty( $registered[ $size ]['height'] ) ? absint( $registered[ $size ]['height'] ) : 0;
			}
		}

		if ( $max_w > 1 || $max_h > 1 ) {
			if ( $full_w > 1 && $full_h > 1 ) {
				[ $constrained_w, $constrained_h ] = wp_constrain_dimensions( $full_w, $full_h, $max_w, $max_h );
				if ( $constrained_w > 1 && $constrained_h > 1 ) {
					return [ absint( $constrained_w ), absint( $constrained_h ) ];
				}
			}
			if ( $max_w > 1 && $max_h > 1 ) {
				return [ $max_w, $max_h ];
		...
		// 3. Fall back to the original image.
		if ( $full_w > 1 && $full_h > 1 ) {
	...
	/**
	 * Make sure attachment image markup carries real width/height attributes.
	 *
	 * Only touches markup that came out with the "1x1" placeholder dimensions (or none at all),
	 * so healthy images keep whatever WordPress generated for them.
	 *
	 * @param string       $html          Image markup generated by wp_get_attachment_image().
	 * @param int          $attachment_id Attachment ID the markup belongs to.
	 * @param string|int[] $size          Image size requested for that markup.
	 *
	 * @return string
	 */
	static function normalize_attachment_image_dimensions( $html, $attachment_id, $size = 'rtcl-thumbnail' ) {
		if ( empty( $html ) || false === strpos( $html, '<img' ) ) {
			return $html;
		}

		preg_match( '/<img[^>]*?\swidth=["\'](\d+)["\']/i', $html, $width_match );
		preg_match( '/<img[^>]*?\sheight=["\'](\d+)["\']/i', $html, $height_match );

		$current_w = isset( $width_match[1] ) ? absint( $width_match[1] ) : 0;
		$current_h = isset( $height_match[1] ) ? absint( $height_match[1] ) : 0;

		// Anything above the 1px placeholder is trusted as is.
		if ( $current_w > 1 && $current_h > 1 ) {
			return $html;
		}

		[ $width, $height ] = self::get_attachment_image_dimensions( $attachment_id, $size );

		if ( $width < 2 || $height < 2 ) {
			return $html;
		}

		foreach ( [ 'width' => $width, 'height' => $height ] as $attribute => $value ) {
			$pattern = '/(<img[^>]*?\s' . $attribute . '=["\'])\d*(["\'])/i';

			if ( preg_match( $pattern, $html ) ) {
				$html = preg_replace( $pattern, '${1}' . $value . '${2}', $html, 1 );
			} else {
				$html = preg_replace( '/<img\s/i', sprintf( '<img %s="%d" ', $attribute, $value ), $html, 1 );
			}
		}
