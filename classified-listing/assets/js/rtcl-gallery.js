(function() {
  ...
  RTCL.File = {
    ...
    GetMime: function(file) {
      ...
      var types = {
        image: ["image/jpeg", "image/jpe", "image/jpg", "image/png", "image/webp"]
      };
  ...
  RTCL.File.Uploader = function(setup) {
    ...
    jQuery.each(setup.data, function(index, result) {
      var file = { id: "rtcl-file-" + result.attach_id };
      $this.FileAdded(null, file);
      $this.FileUploaded(file, result);
    });
    new Sortable(this.sortable.get(0), {
      animation: 150,
      onEnd: jQuery.proxy(this.SortableUpdate, this)
    });
  ...
  RTCL.File.Uploader.Plupload.prototype.FilesAdded = function(up, files) {
    var max = parseInt(this.setup.init.max_files, 10) || 5, total_image = this.GetFileCount(), remaining = max - total_image, filesToUploaded = files;
    if (remaining) {
