      ...
      if (rules[ruleKey].value) {
        ...
        if ("required" === ruleKey) {
          if (field.element === "pricing") {
            ...
            hasError = typeof value === "string" ? getNormalizedTextLength(value) === 0 : !value;
          }
        } else if ("min" === ruleKey) {
          if ("number" === field.element) {
            ...
            let ruleValue = parseInt(rules[ruleKey].value, 10);
            if (value) {
              const checkLength = getNormalizedTextLength(value);
...
const getNormalizedTextLength = (value, field) => {
  ...
  const tmp = document.createElement("div");
  tmp.innerHTML = value;
  const text2 = tmp.textContent || tmp.innerText || "";
