(function() {
  ...
  (function($) {
    ...
    $(document.body).on("rtcl_add_error_tip", function(e, element, error_type) {
      ...
      var regex = new RegExp("[^-0-9%\\" + rtcl.decimal_point + "]+", "gi"), error = "i18n_mon_decimal_error";
      var value = $(this).val();
      ...
      var regex = new RegExp("[^-0-9%\\" + rtcl.decimal_point + "]+", "gi"), value = $(this).val(), newvalue = value.replace(regex, "");
      if (value !== newvalue) {
        $(this).val(newvalue);
      }
      ...
      let $this = $(this), $wrapper = $this.closest(".rtcl-tags-input-wrap"), searchKey = $this.val();
      let data = {
      ...
      switch (e.keyCode) {
        case 188:
          let word = searchKey.trim().replace(",", "");
        ...
        case 38:
          ...
          return;
        case 13:
          let $activeList = $wrapper.find("ul li[active]");
          if ($activeList.length) {
            let word2 = $activeList.text();
            rtcl_tag_add_word(word2);
      ...
      let $this = $(this), $wrapper = $this.closest("div"), termText = $wrapper.find(".rtcl-tag-term").text();
      $this.closest("div").remove();
      rtcl_tag_remove_word(termText);
    });
    function rtcl_tag_remove_word(word) {
      var $wrapper = $(".rtcl-tags-input-wrap"), $input = $wrapper.find("#rtcl_listing_tag"), currentValues = $input.val();
    ...
    function rtcl_tag_add_word(word) {
      var $wrapper = $(".rtcl-tags-input-wrap"), $input = $wrapper.find("#rtcl_listing_tag"), oldValues = $input.val();
      oldValues = oldValues ? oldValues.split(",").map(function(_item) {
        return _item.trim();
      }) : [];
      oldValues.push(word);
      $input.val(oldValues.join(", "));
      let $content = $('<div><span class="rtcl-tag-term">' + word + '</span><span class="remove">x</span></div>');
    ...
    if ($.fn.validate) {
      ...
      if (submitForm.length) {
        submitForm.validate({
          rules: {
            _rtcl_max_price: { greaterThan: "#rtcl-price" }
          },
          messages: {
            _rtcl_max_price: {
          ...
          submitHandler: function(form) {
            ...
            if (rtcl.recaptcha && rtcl.recaptcha.on && $.inArray("listing", rtcl.recaptcha.on) !== -1) {
              if (rtcl.recaptcha.v === 2 && reCaptchaId !== void 0) {
                ...
                const $captcha_msg = $form.find("#rtcl-listing-g-recaptcha-message");
                ...
                grecaptcha.ready(function() {
                  $form.rtclBlock();
                  grecaptcha.execute(rtcl.recaptcha.site_key, { action: "listing" }).then(function(token) {
            ...
            function submit_form_data(reCaptcha_token) {
              const fromData = new FormData(form), msgHolder = $("<div class='alert rtcl-response'></div>");
              if (reCaptcha_token) {
    ...
    $(document).ready(function() {
      if (rtcl_add_post.character_limit.title) {
        $("#rtcl-title").on("keyup", function() {
          var _self2 = $(this);
          var value = $(this).val();
          var maxLength2 = rtcl_add_post.character_limit.title;
          if (maxLength2) {
            var limitWrap2 = _self2.parents(".form-group").find(".target-limit");
            if (limitWrap2.length) {
              limitWrap2.text(value ? maxLength2 - value.length : maxLength2);
            }
          }
        });
      }
      if (rtcl_add_post.character_limit.description) {
        ...
        $("#description").on("keyup", function(event) {
          ...
          if (limitWrap2.length) {
            ...
            if (remain <= 0) {
              limitWrap2.text(0);
              _self2.val(value.substring(0, maxLength));
            } else {
              limitWrap2.text(remain);
            }
          }
        });
        if (typeof tinyMCE !== "undefined") {
          ...
          if (_tinyEditor) {
            let calculateContent2 = function(event) {
              let _content = _tinyEditor.getContent();
            ...
            _tinyEditor.on("keypress", calculateContent2);
            _tinyEditor.on("keyup", calculateContent2);
            _tinyEditor.on("change", calculateContent2);
          }
        }
      }
      $("#rtcl-ad-type").on("change", function() {
        var _self2 = $(this), type = _self2.val(), msgHolder = $("<div class='alert rtcl-response'></div>"), selfWrapper = _self2.parents("#rtcl-ad-type-selection"), target = $("#rtcl-ad-category-selection"), sub_category_wrap = $("#rtcl-sub-category-wrap"), sub_cat_row = $("#sub-cat-row"), data = {
        ...
        if (type) {
          $.ajax({
            ...
            success: function(response) {
              _self2.next(".rtcl-spinner").remove();
              sub_cat_row.addClass("rtcl-hide");
              sub_category_wrap.html("");
              let msg = "";
              if (response.message.length) {
      ...
      $("#rtcl-category").on("change", function() {
        let self = $(this), target = self.parents(".rtcl-post-category"), sub_category_wrap = $("#rtcl-sub-category-wrap"), sub_cat_row = $("#sub-cat-row"), term_id = $(this).val(), type_field = $("#rtcl-ad-type"), type = type_field.val() || "", msgHolder = $("<div class='alert rtcl-response'></div>"), data = {
              ...
              success: function success(response) {
                ...
                if (response.success) {
                  if (response.child_cats) {
                    ...
                    sub_category_wrap.html("");
                    sub_cat_row.addClass("rtcl-hide");
                    let uri = rtcl_add_post.form_uri.toString(), params = {
                      "category": term_id
                    }, glue = uri.indexOf("?") !== -1 ? "&" : "?";
      ...
      $(document).on("change", "#rtcl-sub-category-wrap select", function() {
        let self = $(this), target = self.parents("#rtcl-sub-category-wrap"), term_id = $(this).val(), type_field = $("#rtcl-ad-type"), type = type_field.val() || "", msgHolder = $("<div class='alert rtcl-response'></div>"), data = {
          "action": "rtcl_get_one_level_category_select_list",
          "term_id": term_id,
          __rtcl_wpnonce: rtcl.__rtcl_wpnonce
        };
      ...
      $("#rtcl-location").on("change", function() {
        var self = $(this), subLocation = $("#rtcl-sub-location"), subLocationRow = subLocation.parents("#sub-location-row"), data = {
        ...
        $.ajax({
          ...
          success: function(data2) {
            ...
            subLocation.append(data2.locations);
            if (data2.locations) {
      ...
      $("#rtcl-sub-location").on("change", function() {
        var self = $(this), subSubLocation = $("#rtcl-sub-sub-location"), subSubLocationRow = subSubLocation.parents("#sub-sub-location-row"), data = {
          "action": "rtcl_get_sub_location_options",
          "term_id": $(this).val(),
          __rtcl_wpnonce: rtcl.__rtcl_wpnonce
        };
        $.ajax({
          ...
          success: function(data2) {
            ...
            subSubLocation.append(data2.locations);
            if (data2.locations) {
